...
Qonversion validates user receipts with app stores and sends subscription events to your HTTP endpoints. Keep in mind events delivery time when using webhooks to manage subscriber statuses on your back-end. Webhook events are delivered within 20 to 120 seconds of the event occurring. Please consider using [checkEntitlements](check-permissions) method of Qonversion SDK to get real-time subscription status on the client-side.
...
![](/guides/images/docs/ce45d70-Webhooks_integration.png)
...
| properties                 | yes      | User properties provided by SDK. Read more about user properties[here](user-properties)                                                                                                                                         |
...
| entitlements               | yes      | Array of granted user entitlements. Entity's fields are synchronized with our[public API](/guides/reference/overview).                                                                   |
