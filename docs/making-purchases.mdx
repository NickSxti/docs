...
Make sure to [configure Products, Entitlements and Offerings](/docs/subscription-management-mode) in the Qonversion dashboard before you start handling purchases with Qonversion SDK:
...
Where **"product"** is the Qonversion Product created in the [Dashboard](https://dash.qonversion.io/entitlements/products). See the [previous step](/docs/displaying-products) to display Products.
...
Entitlement IDs are the keys to the `entitlements` dictionary. The values are the objects of the [Qonversion.Entitlement](/docs/check-permissions#the-entitlement-object) class.
...
Use the `checkEntitlements()` SDK method in case you want to check users’ entitlements separately from a purchase. Learn more [here](/docs/check-permissions).
