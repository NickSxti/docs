...
1. At the top of the processing funnel, Splitter receives User\_A.
   * Splitter receives User\_A when the [`remoteConfig` method](https://documentation.qonversion.io/docs/launch-experiments#get-your-configuration) is called
...
4. If User\_A matches segmentation rules, he is assigned to a specific group inside the experiment. Otherwise, Splitter returns to Step #2 to check User\_A against the following experiment in the queue.
   1. Please note, if you have the traffic level in the first launched experiment (sorted by start date) set to 70% and in the second - 30%, that means that your second experiment is exposed for (100% - 70%) \* 30% = 9% of all users. Set the traffic level to 100% for the second experiment in case you want to use all the remaining from the first experiment users. [Below](https://documentation.qonversion.io/docs/splitting-rules#example-1) you can find some examples that help understand this approach better.
...
<Info>
  ...
  To turn off exposure to cross-context experiments, please use the [Attached active experiment and Attached experiment context key](https://documentation.qonversion.io/docs/launch-experiments#segment-your-users) segmentation options.
</Info>
...
Based on the [General splitting rules](https://documentation.qonversion.io/docs/splitting-rules#general-splitting-rules), you have the following distribution picture:
