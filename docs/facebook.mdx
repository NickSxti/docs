...
3. Additionally, to attribute events sent from Qonversion, you need to set the same user ID at `_q_custom_user_id` key using the `Qonversion.shared().setProperty` method. Follow the [documentation](/docs/user-properties) to learn more about this method.
...
You can use the default event names provided by Qonvresion and corresponding to Facebook standard event names. If the [standard Facebook event names](/docs/facebook) are used, the events will show up in your Facebook Ads Account. If custom event names are used they will show up in Facebook Analytics, but they will not be delivered to Facebook Ads.
