...
Below you can find a simple step-by-step guide on how to migrate to Qonversion for both [Analytics and Subscription management modes](/docs/quickstart).
...
1. **Integrate Qonversion SDK** to track analytics and entitlements for your new subscribers using our [quick start guide](/docs/quickstart).
2. Сall the [*syncHistoricalData*](/docs/migrating-subscriptions#sdks-synchistoricaldata-method) method right after Qonversion SDK initialization to synchronize all the device-related data and ensure that none of the entitlements has been missed.
...
1. Prepare files with [AppStore with Base64 encoded receipt data](/docs/migrating-subscriptions#app-store-data-migration-file) or [Google Play with purchase tokens](/docs/migrating-subscriptions#google-play-data-migration-file).
   ...
   2. Additionally, but not necessarily, to keep your historical data granular, provide us with such user attributes as [Country, IDFV, IDFA, GAID, AAID, etc.](/docs/migrating-subscriptions#additional-user-related-fields-for-analytics).
...
Ensure you [have configured Entitlements and Products](/docs/subscription-management-mode#1-configure-products--entitlements-permissions) before you start users' entitlements migration process, so Qonversion can determine what entitlements should be granted to a user based on product ID of the purchased product.
