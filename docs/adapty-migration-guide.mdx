...
1. **App-Specific Shared Secret**: Copy this value from the App Store Connect shared secret field in the App Settings -> iOS SDK section in the Adapty dashboard. [Here's](/docs/app-specific-shared-secret) a more detailed guide on how to obtain this.
...
2. **Service Account Credentials JSON**: Copy the value from the **Service account key file** shown in the screenshot above, or attach the same file you used with Adapty. [Here's more](/docs/service-account-key-android) on how you can get Google Play Service Account Key.
...
* **Google Play Product ID** – product identifier on Google Play Console. Learn more on how to set up [Android in-app products ](/docs/android-in-app-products).
...
See more information [here](/docs/server-to-server-notifications).
...
It takes up to 24 hours for your [Google service credentials](/docs/service-account-key-android) to work correctly. Make sure to wait for at least 24 hours after you have created the credentials before connecting to Google Real-Time Developer Notifications.

See more information [here](/docs/google-developer-notifications).

# Install SDKs

Check out the links below for the SDK you need and follow the steps provided:

→ [iOS SDK](/docs/ios-sdk-setup) → [Android SDK](/docs/android-sdk) → [Flutter SDK](/docs/flutter-sdk) → [React Native SDK](/docs/react-native-sdk) → [Unity SDK](/docs/unity-sdk) → [Cordova Plugin](/docs/cordova) → [Capacitor Plugin](/docs/capacitor) → [Web](/docs/web-sdk) → [Kids Mode Qonversion SDKs](/docs/kids-mode-sdk)

Read more about [Installing the SDKs](/docs/install-sdk).
...
[See more about Offerings](/docs/offerings)
