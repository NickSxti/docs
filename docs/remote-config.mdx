...
<Info>
  ...
  Qonversion Remote Configs and Experiments share single methods to get configurations. You can distinguish the source of the result received by looking at the *source* field. [Learn more about fields available](/docs/remote-config#remoteconfig-fields-description).
</Info>
...
* Qonversion assigns eligible users to launched experiments once the `remoteConfig` is called. We also recommend paying attention to the [Segmentation rules](/docs/remote-config#2-segment-users) for the configuration.

# 4. Test changes before launch

<Info>
  Please, note, the guide below allows you to test only configurations created through the [Remote Config dashboard section](https://dash.qonversion.io/remote-configs). To test configurations from the [Experiments section](https://dash.qonversion.io/ab-testing), please, follow the [test experimental changes before launch](/docs/launch-experiments#test-experimental-changes-before-launch) guide.
</Info>
...
To enhance the reliability of remote configurations, consider utilizing Qonversion [fallback files](/docs/system-reliability#/fallback-files). These files will serve as a backup in case of network provider errors or system failures, ensuring that your application continues to receive essential data and avoids any downtime. For more information on fallback files and how to install them, please refer to [this page](/docs/system-reliability#/fallback-files).
