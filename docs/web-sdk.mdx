...
User properties are attributes you can set on a user level. You can send user properties to third-party platforms and use them in Qonversion for customer segmentation and analytics. You can read more about user properties [here](https://documentation.qonversion.io/docs/user-properties).
...
For now, Qonversion Web SDK accepts only [Stripe](doc:stripe-integration) purchases. We are working on supporting more billing providers. To send the purchase to Qonversion after you receive a successful response from Stripe, call the following method and provide the received data.
...
To let Qonversion handle user entitlements, you should configure them in the [Qonversion dashboard](https://documentation.qonversion.io/docs/subscription-management-mode#1-configure-products--entitlements) first. After you have it done, Qonversion will grant entitlements to the current user each time you send us a successful purchase. Then you can get the current user entitlements as follows.
...
User Identity allows cross-device & cross-platform user identification and access management. You can read more about it [here](https://documentation.qonversion.io/docs/user-identifiers#3-user-identity).
...
Consider looking at the examples [here](https://documentation.qonversion.io/docs/user-identifiers#examples) to better understand `identify()` at work.
