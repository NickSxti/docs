...
1. **App-Specific Shared Secret**: Copy this value from the App Store Connect App-specific Shared Secret field in the App Settings app settings in the RevenueCat dashboard. See the screen below. [Here's](/docs/app-specific-shared-secret) a more detailed guide on how to obtain this.
...
2. **Service Account Credentials JSON**: attach the same file you used for **Service Account Credentials JSON** at RevenueCat. [Here's more](/docs/service-account-key-android) on how you can get Google Play Service Account Key.
